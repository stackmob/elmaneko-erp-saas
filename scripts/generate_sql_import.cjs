...
const impRows = data.impressoras.map(i => {
  ...
  const status = (i.status === 'ativa' || i.status === 'Disponível') ? 'Ativa' : (i.status || 'Ativa');
...
VALUES
`;
function mapFormaPagamento(val) {
  if (!val) return 'Pix';
  const v = String(val).trim().toLowerCase();
  if (v.includes('pix')) return 'Pix';
  if (v.includes('dinheiro') || v.includes('especie')) return 'Dinheiro';
  if (v.includes('credito') || v.includes('crédito')) return 'Cartão de Crédito';
  if (v.includes('debito') || v.includes('débito')) return 'Cartão de Débito';
  if (v.includes('boleto')) return 'Boleto';
  return 'Pix';
}

function mapStatusVenda(val) {
  if (!val) return 'Pago';
  const s = String(val).trim().toLowerCase();
  if (s === 'pago' || s === 'concluído' || s === 'concluido' || s === 'aprovado' || s === 'finalizado') return 'Pago';
...
const venRows = data.vendas.map(v => {
  ...
  const formaPagamento = mapFormaPagamento(v.formaPagamento);
  const status = mapStatusVenda(v.status);
  return `  (${escapeSql(uuid)}, ${escapeSql(EMPRESA_ID)}, ${escapeSql(cliUuid)}, ${escapeSql(v.data || '2026-01-01')}, ${escapeSql(v.valorTotal || 0)}, ${escapeSql(formaPagamento)}, ${escapeSql(status)}, ${escapeSql(orcUuid)})`;
});
